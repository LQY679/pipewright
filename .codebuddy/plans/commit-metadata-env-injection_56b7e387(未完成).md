---
name: commit-metadata-env-injection
overview: 扩展 Pipewright 的 Git 提交元数据，将提交作者、提交备注、提交时间通过 COMMIT_AUTHOR/COMMIT_MESSAGE/COMMIT_TIME 环境变量注入到本地隔离容器与远程 SSH 构建两种执行环境的 script 步骤，不改变现有其他功能。
todos:
  - id: extend-cloneresolved
    content: 扩展 internal/build/cloner.go 的 CloneResolved 新增 Author/Message/Time 字段并在 Clone 中解析填充
    status: pending
  - id: add-commit-meta-env
    content: 在 internal/build/dag_stage_exec.go 新增 commitMetaAsEnv 辅助函数
    status: pending
    dependencies:
      - extend-cloneresolved
  - id: inject-local-remote
    content: 在 dag_stage_exec.go 与 remote_stage_exec.go 的 script env 组装处追加 commitMetaAsEnv
    status: pending
    dependencies:
      - add-commit-meta-env
  - id: verify-build-test
    content: 执行 go build 与 go test ./internal/build/... 验证无回归并补注入断言
    status: pending
    dependencies:
      - inject-local-remote
---
...
3. **解析位置**：在 `(*Cloner).Clone` 成功克隆/检出后，取 `head, _ := repo.Head()`，再 `commitObj, _ := repo.CommitObject(head.Hash())`。go-git 浅克隆（Depth:1）HEAD 可达，CommitObject 可用，无需改克隆策略，避免额外网络开销。解析失败（极端情况）仅留空字段，不影响既有 CommitShort 与构建。
4. **注入辅助函数**：新增 `commitMetaAsEnv(resolved *CloneResolved) []pipeline.BuildVar`，字段为空跳过，返回明文（Secret:false）BuildVar，统一复用 `pipeline.BuildVar{Key, Value, Secret:false}` 结构，与 `runParamsAsEnv` 风格一致。
5. **透传方式**：`cloneJobWorkspace` 当前仅返回 commitTag；为保持改动聚焦，注入点直接复用调用处已存在的 `resolved` 局部变量（dag_stage_exec.go 第186-201行、第486-501行作用域内已有 resolved），把 `commitMetaAsEnv(resolved)` 插在 `settings.Build.Vars` 之后、`step.Env` 之前，满足「job 自身 env 可覆盖同名」语义。remote_stage_exec.go 第88行 resolved 已在作用域，第128行处追加即可。
6. **两路径一致**：本地 dag_stage_exec.go（两处 script 注入点）与 remote_stage_exec.go（远程 SSH 路径）都追加，保证执行环境一致。
